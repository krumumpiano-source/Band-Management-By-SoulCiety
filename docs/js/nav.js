...
function renderMainNav(containerId) {
  ...
  function navSubmenu(icon, title, desc, pages, items) {
    var isOpen = pages.some(function(p) { return p === currentPage; });
    return '<li>' +
      '<div class="nav-submenu-toggle' + (isOpen ? ' open' : '') + '" onclick="this.classList.toggle(\'open\');this.nextElementSibling.classList.toggle(\'open\')">' +
        '<div>' + icon + ' ' + title + (desc ? '<span class="nav-link-desc">' + desc + '</span>' : '') + '</div>' +
        '<span class="arrow">▼</span>' +
      '</div>' +
      '<ul class="nav-submenu-items' + (isOpen ? ' open' : '') + '">' + items + '</ul>' +
    '</li>';
  }
  ...
  var _adminPage = (currentPage === 'admin' || currentPage === 'admin-songs' || currentPage === 'member-worklog');
  ...
  th: {
    ...
    member: {
      ...
      items: [
        ...
        { q: 'วิธีดูเพลง / เซ็ตลิสต์', a: '🎵 เมนู "คลังเพลง" (Songs)\nดูเพลงทั้งหมดในคลัง และเซ็ตลิสต์ประจำวัน' },
      ]
    },
    manager: {
      label: '👔 ผู้จัดการวง',
      items: [
        ...
        { q: 'วิธีบันทึกการเงิน / เบิกเงิน', a: '📋 เมนู "เบิกจ่าย & ลงเวลา" (Attendance & Payroll)\nเช็คชื่อเข้างาน และเบิกจ่ายค่าตัวอัตโนมัติ' },
        { q: 'วิธีคำนวณราคารับงาน', a: '🧮 เมนู "คำนวณราคางาน" (Job Calculator)\nกรอก: จำนวนชั่วโมง จำนวนสมาชิก ค่าเดินทาง ค่าอุปกรณ์\nระบบจะคำนวณราคาแนะนำให้ทันที' },
